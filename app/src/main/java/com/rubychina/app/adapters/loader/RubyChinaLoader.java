 ...
 */
public class RubyChinaLoader extends BaseLoader<ITopic> {

    public RubyChinaLoader(LoaderCallback<ITopic> callback) {
        super(callback);
    }

    @Override
    public void loadData() {
        JsonUTF8ArrayRequest req = new JsonUTF8ArrayRequest(RubyChinaService.instance.getTopicListApi(),
                new Response.Listener<JSONArray>() {
                    ...
                    public void onResponse(JSONArray response) {
                        ...
                        List<com.rubychina.app.entities.rubychina.Topic> topics = gson.fromJson(response.toString(), listType);
                        ArrayList<ITopic> list = new ArrayList<ITopic>() ;
                        for(int i=0;i<topics.size();i++){
                            list.add(topics.get(i));
                        }
                        _listdataLoadedCallback.onDataLoaded(list,RubyChinaService.instance);
